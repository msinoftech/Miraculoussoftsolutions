    ...
    <div className="relative z-10">
      ...
      <div className="mt-8 grid gap-4 sm:grid-cols-3">
        {item.points.map((point) => (
          <div key={point} className="rounded-xl border border-zinc-200 bg-zinc-50 p-4">
            <span>
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="lucide lucide-circle-check-big-icon lucide-circle-check-big text-red-600">
                <path d="M21.801 10A10 10 0 1 1 17 3.335" />
                <path d="m9 11 3 3L22 4" />
              </svg>
            </span>
            <div className="mt-2 text-[15px] font-medium text-zinc-900">{point}</div>
          </div>
        ))}
      </div>
      ...
      <div className="mt-9 rounded-xl bg-zinc-950 p-6 text-white">
        <div className="mb-4 flex gap-3">
          ...
          <div>
            {signalTitle && <div className="text-[11px] font-bold uppercase tracking-[0.12em] text-white">{signalTitle}</div>}
            {signalText && <div className="text-[18px] font-bold tracking-[-0.02em]">{signalText}</div>}
          </div>
        </div>
        ...
        <div className="space-y-4 lg:hidden">
          {items.map((step, index) => {
            ...
            return (
              <div
                key={step.id}
                className={`overflow-hidden rounded-xl border transition ${isActive ? "border-zinc-950 bg-white shadow-2xl shadow-zinc-950/10" : "border-zinc-200 bg-white"}`}
              >
                <button
                  type="button"
                  onClick={() => toggleMobileAccordion(index)}
                  aria-expanded={isActive}
                  className={`group w-full p-5 text-left transition ${isActive ? "bg-zinc-950 text-white" : "text-zinc-950 hover:border-red-300"}`}
                >
