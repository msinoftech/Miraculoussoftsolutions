      ...
      <div className="relative z-10 w-[335px] rounded-[2.2rem] border border-slate-200 bg-white/95 p-5 shadow-2xl shadow-slate-300/60 backdrop-blur-2xl">
        <div className="mb-4 flex items-center justify-between">
          <div className="flex gap-3">
            ...
            <div>
              <div className="text-sm font-bold leading-[1.05] tracking-[-0.03em] text-zinc-950">Education Center</div>
              <div className="text-sm text-zinc-500">Live institutional overview</div>
            </div>
          </div>
        ...
        <div className="grid grid-cols-2 gap-3 pb-3">
          {["4.8k Students", "320 Staff", "18 Branches", "92% Progress"].map((v) => (
            <div key={v} className="rounded-2xl bg-slate-100 p-3 text-xs font-bold text-slate-700">{v}</div>
          ))}
        </div>
