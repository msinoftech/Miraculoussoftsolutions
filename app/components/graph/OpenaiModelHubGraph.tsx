...
type OpenaiModelHubGraphProps = {
  className?: string;
};

export default function OpenaiModelHubGraph({ className = "" }: OpenaiModelHubGraphProps) {
  ...
  return (
    <div className={`service-panel-frame service-panel-grid relative w-full hidden lg:block ${className}`.trim()}>
      <div className="relative">
        <div className="pointer-events-none absolute left-1/2 top-[38%] h-48 w-48 -translate-x-1/2 rounded-full blur-3xl sm:h-56 sm:w-56" style={{ backgroundColor: `${activeModel.accent}18` }} />
        ...
        <div className="relative mx-auto aspect-square w-full">
          ...
          <svg className="absolute inset-0 z-0 h-full w-full" viewBox="0 0 100 100" preserveAspectRatio="xMidYMid meet" aria-hidden>
            ...
            {openaiGraphPositions.map((pos, i) => (
              <line
                ...
                x2={pos.x}
                y2={pos.y}
                stroke={i === activeIndex ? OPENAI_MODELS[i].accent : `url(#${gradientId})`}
          ...
          <div className="absolute left-1/2 top-1/2 z-1 w-[250px] -translate-x-1/2 -translate-y-1/2 overflow-hidden rounded-xl border border-zinc-200 bg-zinc-950 shadow-xl">
            ...
            <div className="space-y-1 p-3 leading-relaxed text-sm">
              <div style={{ color: activeModel.accent }}>✔ Model → {activeModel.name} ({activeModel.provider})</div>
